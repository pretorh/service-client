...
function get(url, options, callback) {
    ...
    function start() {
        if (options.protocol == "") {
            options.protocol = urlinfo.protocol ? urlinfo.protocol.replace(/:$/, "") : null;
        }
        var request;
        if (options.protocol === "https") {
            request = options.https(requestOptions, onResponse);
        } else {
            request = options.http(requestOptions, onResponse);
        }
    ...
    function resizeBuffer(mustBe) {
        if (buffer.length < mustBe) {
            ...
            buffer.copy(newBuf, 0, 0, received);
            buffer = newBuf;
        }
    }
