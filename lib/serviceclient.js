...
function get(url, options, callback) {
    ...
    function start() {
        var request = http.request(requestOptions, onResponse);
        request.on("error", doCallback);
        request.end();
    }
    
    function onResponse(response) {
        if (Math.floor(response.statusCode / 100) != 2) {
            doCallback(new Error("non 2xx status code: " + response.statusCode));
        } else {
    ...
    function transform() {
        options.transform(parsed, doCallback);
    }
